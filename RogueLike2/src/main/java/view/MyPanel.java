...
public class MyPanel {
    ...
    private void showButtonsOnHorizontal() {
        ...
        for (int i = 0;i < myButtonList.size();i++) {
            ...
            TextColor backgroundColor = i == currentButton ? myButtonList.get(currentButton).getActionColor() : myButtonList.get(i).getColoredText().getBackgroundColor();
            textBuffer.setForegroundColor(myButtonList.get(i).getColoredText().getColor());
            textBuffer.setBackgroundColor(backgroundColor);
            textBuffer.putString(new TerminalPosition(posX, panelPosition.getRow() + 2), myButtonList.get(i).getColoredText().getText());
            posX += myButtonList.get(i).getColoredText().getText().length() + realSpacing;
        }
    }
    ...
    private void showButtonsOnVertical() {
        ...
        for (int i = 0;i < myButtonList.size();i++){
            ...
            textBuffer.setForegroundColor(myButtonList.get(i).getColoredText().getColor());
            textBuffer.setBackgroundColor(backgroundColor);
            textBuffer.putString(new TerminalPosition(panelPosition.getColumn() + (panelSize.getColumns() - myButtonList.get(i).getColoredText().getText().length()) / 2, posY), myButtonList.get(i).getColoredText().getText());
            posY += 2;
        }
    }
    ...
    private void drawPanel() {
        ...
        clearPanel(true);
        textBuffer.drawRectangle(panelPosition, panelSize, TextCharacter.fromCharacter(' ', themeStyle.getForeground(), themeStyle.getForeground())[0]);
        textBuffer.fillRectangle(new TerminalPosition(panelPosition.getColumn() + 1, panelPosition.getRow() + 1), new TerminalSize(panelSize.getColumns() - 2, panelSize.getRows() - 2), TextCharacter.fromCharacter(' ', themeStyle.getBackground(), themeStyle.getBackground())[0]);
        Print.drawRectangle(textBuffer,themeStyle.getForeground(), TextColor.ANSI.DEFAULT, 0, 0, screen.getTerminalSize().getColumns(), screen.getTerminalSize().getRows());
    }
    ...
    private TerminalSize calculatePanelSize() {
        ...
        if (plane == Plane.VERTICAL) {
            for (MyButton button : myButtonList) {
                String temp = button.getColoredText().getText();
    ...
    public int getButtonsLength(){
        int totalLength = 0;
        for (MyButton button : myButtonList){
            totalLength += button.getColoredText().getText().length();
        }
        return totalLength;
    }
